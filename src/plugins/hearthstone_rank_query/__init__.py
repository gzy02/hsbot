...
async def solve(event:MessageEvent,rank_query:Type[Matcher], keyword: str, mode: str):
    ...
    if res_post_data['status'] == 1:
        ...
        if res['status'] == 1:
            ...
            await rank_query.send(message=f"hsbot与hs.blizzard.cn的网络连接不佳，请联系系统管理员[QQ:{SYSTEM_ADMIN_QQ_NUMBER}]")
    else:
        await rank_query.send(message=f"hsbot与hs.blizzard.cn的网络连接不佳，请联系系统管理员[QQ:{SYSTEM_ADMIN_QQ_NUMBER}]")
...
async def _rank_query(bot: Bot, event: MessageEvent):
    try:
        ...
        await rank_query.finish(f'[{text_list[0]}]程序错误，请联系系统管理员[QQ:{SYSTEM_ADMIN_QQ_NUMBER}]\n错误如下：\n{repr(e)}')
