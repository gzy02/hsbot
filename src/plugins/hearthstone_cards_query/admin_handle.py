...
async def _reset_database(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        await reset_database.finish(
            (f'[重置数据库]程序错误，请联系系统管理员[QQ:{SYSTEM_ADMIN_QQ_NUMBER}]\n错误如下：\n{repr(e)}'))
...
async def _all_set_query(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        await all_set_query.finish(
            (f'[查系列]程序错误，请联系系统管理员[QQ:{SYSTEM_ADMIN_QQ_NUMBER}]\n错误如下：\n{repr(e)}'))
...
async def _delete_admin(bot: Bot, event: MessageEvent):
    ...
    try:
        if str(SYSTEM_ADMIN_QQ_NUMBER) != event.get_user_id():  #得是系统管理员
            ...
            if len(text_list) >= 2:
                for i in range(1, len(text_list)):  #存在的才删去
                    if text_list[i] in admin_qq_number["admin_qq_number"]:
                        admin_qq_number["admin_qq_number"].remove(text_list[i])
                with open(admin_qq_number_json_path, "w", encoding="utf8") as fp:
                    fp.write(str(admin_qq_number).replace("'", '"'))  #JSON格式，单引号换双引号
                await delete_admin.send(f'已删除上述管理员')
            else:
                await delete_admin.send(f'您的输入有误，请输入“删除管理员 <管理员qq号列表>”')
    except Exception as e:
        await delete_admin.finish(
            (f'[删除管理员]程序错误，请联系系统管理员[QQ:{SYSTEM_ADMIN_QQ_NUMBER}]\n错误如下：\n{repr(e)}'))
...
async def _add_admin(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        await add_admin.finish(
            (f'[添加管理员]程序错误，请联系系统管理员[QQ:{SYSTEM_ADMIN_QQ_NUMBER}]\n错误如下：\n{repr(e)}'))
...
async def _admin_query():
    ...
    try:
        ...
        await admin_query.finish(
            (f'[查询管理员]程序错误，请联系系统管理员[QQ:{SYSTEM_ADMIN_QQ_NUMBER}]\n错误如下：\n{repr(e)}'))
...
async def _reset_jjcCardsSet(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        await reset_jjcCardsSet.finish(
            (f'[重置jjc]程序错误，请联系系统管理员[QQ:{SYSTEM_ADMIN_QQ_NUMBER}]\n错误如下：\n{repr(e)}'))
